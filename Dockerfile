...
RUN go get && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "-X github.com/gbbirkisson/rpi.Version=$(git rev-parse HEAD)" -a -installsuffix cgo -o /go/bin/rpi-server
